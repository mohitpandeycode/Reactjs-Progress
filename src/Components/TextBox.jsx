        ...
        <>
            <div className={`container mt-3 text-${props.mode === 'light' ? 'dark' : 'light'}`}>
                ...
                <textarea className="form-control"
                    style={{
                        backgroundColor: props.mode === "dark" ? '#0a1433' : 'white',
                        color: props.mode === "dark" ? 'white' : 'black'
                    }} onChange={HandleOnChange} onClick={clickbox} value={Text} rows="8"></textarea>
                {Text === "" || Text === "Enter the text here..." ? null :
                    (<div>
                        ...
                        <button type="button" onClick={HandleClear} className="btn btn-primary m-2 btn-lg">Clear Text</button>
                    </div>
                    )}
            </div>
            ...
            <div className={`container my-3 text-${props.mode === 'light' ? 'dark' : 'light'}`}>
                ...
                <p><b>{Text.split(" ").filter((e) => { return e !== '' }).length} </b>Words and <b>{Text.length} </b>Characters</p>
            </div>
        </>
    )
}
